...
Deno.serve(async (req) => {
  ...
  // Extract path after /functions/v1/make-server-8fff4b3c
  ...
  console.log(`[MongoDB Server] [${req.method}] ${path}`);
  ...
  try {
    ...
    if (path === '/archive/restore' && req.method === 'POST') {
      ...
      const archiveEntry = await archiveCollection.findOne({ id: body.archiveId });
      ...
      // Restore to original collection based on entityType
      const targetCollection = archiveEntry.entityType === 'number' 
        ? Collections.NUMBERS_DATABASE 
        : Collections.PROMOTIONS;
      
      const collection = await getCollection(targetCollection);
      await collection.insertOne(archiveEntry.data);
      
      // Remove from archive
      await archiveCollection.deleteOne({ id: body.archiveId });
    ...
    // ==================== GLOBAL SETTINGS ====================
    if (path === '/settings/global-target' && req.method === 'GET') {
      const collection = await getCollection(Collections.GLOBAL_SETTINGS);
      const settings = await collection.findOne({ type: 'global' });
      const target = settings?.dailyTarget || 30;
      return new Response(
    ...
    if (path === '/settings/global-target' && req.method === 'POST') {
      const body = await req.json();
      const collection = await getCollection(Collections.GLOBAL_SETTINGS);
      
      await collection.updateOne(
        { type: 'global' },
        { $set: { dailyTarget: body.target, updatedAt: new Date().toISOString() } },
        { upsert: true }
      );
    ...
    // ==================== EMAIL TEMPLATES ====================
    if (path === '/email-templates' && req.method === 'GET') {
      const collection = await getCollection(Collections.GLOBAL_SETTINGS);
      const settings = await collection.findOne({ type: 'email_templates' });
    ...
    if (path === '/email-templates' && req.method === 'POST') {
      const body = await req.json();
      const collection = await getCollection(Collections.GLOBAL_SETTINGS);
      
      await collection.updateOne(
        { type: 'email_templates' },
        { $set: { templates: body.templates, updatedAt: new Date().toISOString() } },
        { upsert: true }
      );
    ...
    if (path === '/send-email' && req.method === 'POST') {
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: 'Email sent successfully (MongoDB)' 
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (path === '/send-quick-email' && req.method === 'POST') {
      const body = await req.json();
      console.log('[EMAIL] Quick send request:', body);
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: 'Quick email sent (MongoDB)' 
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (path === '/stats' && req.method === 'GET') {
      ...
      const [totalUsers, totalNumbers, totalAssignments, totalCalls] = await Promise.all([
        usersCollection.countDocuments(),
        numbersCollection.countDocuments(),
        assignmentsCollection.countDocuments(),
        callLogsCollection.countDocuments(),
      ]);
